...
class DocumentIngestionPipeline:
    ...
    def _read_document(self, file_path: str) -> tuple[str, Optional[Any]]:
        """
        Lee un documento usando Docling cuando procede (PDF/DOCX/PPTX/HTML/Excel),
        con OCR (EasyOCR por defecto) y descripciones de imágenes vía API (Ollama OpenAI-compatible).
        Devuelve (markdown, docling_document); para audio/texto simple, el segundo es None.
        """
        import os
        import logging
        logger = logging.getLogger(__name__)
        ...
        # 2) Formatos soportados por Docling -> conversión a markdown
        ...
        if file_ext in docling_formats:
            try:
                ...
                from docling.datamodel.pipeline_options import (
                    PdfPipelineOptions,
                    EasyOcrOptions,  # <-- usa el backend concreto
                    PictureDescriptionApiOptions,
                )
                ...
                # OCR (usa una clase de backend concreta: EasyOcrOptions / TesseractCliOcrOptions / RapidOcrOptions / OcrMacOptions)
                pipeline_options.do_ocr = True
                # Idiomas del OCR como lista (CSV en la env: "es,en" etc.)
                ocr_langs_env = os.getenv("DOCLING_OCR_LANGS", "es,en")
                ocr_langs = [x.strip() for x in ocr_langs_env.replace(";", ",").split(",") if x.strip()]
                pipeline_options.ocr_options = EasyOcrOptions(
                    lang=ocr_langs,
                    force_full_page_ocr=(os.getenv("DOCLING_FORCE_FULL_PAGE_OCR", "0") == "1"),
                )

                # Exportar miniaturas de imágenes y escala de export (esto es de salida, no del VLM)
                pipeline_options.generate_picture_images = True
                pipeline_options.images_scale = float(os.getenv("DOCLING_IMAGES_SCALE", "1.0"))

                # ---- Descripción de imágenes por API (OpenAI-compatible; Ollama vía /v1/chat/completions)
                base = os.getenv("DOCLING_VLM_API_URL", "http://ollama:11434")
                if "/v1" not in base:
                    base = base.rstrip("/") + "/v1/chat/completions"

                headers = {}
                extra_headers = os.getenv("DOCLING_VLM_API_HEADERS_JSON", "").strip()
                if extra_headers:
                    import json
                    try:
                        headers.update(json.loads(extra_headers))
                    except Exception:
                        logger.warning("DOCLING_VLM_API_HEADERS_JSON malformado; ignorando.")

                picture_desc_opts = PictureDescriptionApiOptions(
                    url=base,
                    params={
                        "model": os.getenv("DOCLING_VLM_API_MODEL", "llava:7b"),
                        # puedes añadir más params si tu server los soporta (max_completion_tokens, temperature, etc.)
                        "temperature": 0.3,
                        "top_p": 0.2,
                    },
                    prompt=(
                        "Eres un asistente que SOLO responde en español (es-ES), de forma breve, clara y útil para RAG.\n"
                        "Describe la imagen en 3-6 líneas máximo, sin opinión ni florituras. "
                        "Si es un gráfico o tabla, indica variables/ejes y 2-3 valores o tendencias. "
                        "Si es una foto, identifica sujeto, acción, contexto y elementos distintivos. "
                        "No uses portugués ni inglés. No inventes datos no visibles. Responde SOLO en español."
                    ),
                    timeout=int(os.getenv("DOCLING_VLM_API_TIMEOUT", "90")),
                    headers=headers,
                    # filtros umbrales del extractor de imágenes para el VLM:
                    min_area_ratio=float(os.getenv("DOCLING_MIN_AREA_RATIO", "0.0")),  # 0.0 => describe todo
                    min_confidence=float(os.getenv("DOCLING_MIN_CONFIDENCE", "0.0")),
                    # 0.0 => sin filtrar por confianza
                    scale=float(os.getenv("DOCLING_PICTURE_SCALE", "1.0")),  # escala al enviar al VLM
                    provenance="api",  # valor string, no bool
                )
                ...
                # --- Fallback robusto a texto (y saneado) ---
                try:
                    if file_ext == ".pdf":
                    ...
                    try:
                        ...
                        text = f"[Error: Could not read file {os.path.basename(file_path)}]"

                return (text.replace("\x00", " "), None)

        # 3) Texto plano / markdown, etc.
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                return (f.read().replace("\x00", " "), None)
...
def _convert_with_docling(file_path: str) -> tuple[str, object]:
    api_url = os.getenv("DOCLING_VLM_API_URL", "http://ollama:11434/v1/chat/completions")
    api_model = os.getenv("DOCLING_VLM_API_MODEL", "llava:7b")

    pdf_opts = PdfPipelineOptions(
        do_ocr=True,
        ocr_options=OcrOptions(force_full_page_ocr=False),
        do_picture_description=True,
        picture_description_options=PictureDescriptionApiOptions(
            url=api_url,
            model=api_model,
            # headers opcionales si tu endpoint lo requiere
    ...
    result = converter.convert(Path(file_path).resolve())
    markdown = result.document.export_to_markdown()

    # Heurística para texto “mutilado” (ligaduras perdidas)
    if _looks_broken(markdown):
    ...
    markdown = _fix_ligatures(markdown)
    return markdown, result.document  # << devolvemos DoclingDocument

def _looks_broken(text: str) -> bool:
    # muy simple; ajusta a tu gusto
    bad = (" Locaon", " Execuve", "Meeng", "operaons", "applicaon")
    return sum(1 for p in bad if p in text[:4000]) >= 2
